...
void GridSystem::constructCells()
{
	...
	m_center = glm::vec2(m_room->getCentrePosition().x, m_room->getCentrePosition().y);
	for (int z = 0; z < z_nrOfCells; z++)
	{
		for (int x = 0; x < x_nrOfCells; x++)
		{
			glm::vec2 cellPos = m_center + glm::vec2((x * m_cellSize) - m_width * 0.5, (z * m_cellSize) - m_length * 0.5);
			bool valid = true;
			// Check collision
			for (int i = 0; i < boundingBoxes.size() && valid; i++)
			{
				AABB* bb = boundingBoxes[i];
				valid = bb->checkCollisionWithCell(cellPos, m_cellSize * 0.25f);
			}
...
{
	int cx = ((x - m_center.x) + m_width * 0.5) / m_cellSize;
	int cz = ((z - m_center.y) + m_length * 0.5) / m_cellSize;
